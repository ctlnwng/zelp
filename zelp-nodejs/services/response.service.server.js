module.exports = function(app) {
  ...
  function voteResponse(req, res) {
    ...
    responseModel.findUserVote(rid, userId).then(responses => {
      ...
      if (responses.length > 0) {
        ...
        if (voteType === 1) {
          responseModel
            .incrementVotes(rid, 1)
            .then(() => responseModel.addVote(rid, newVote))
            .then(resp => res.json(resp));
        } else if (voteType === 0) {
          responseModel
            .decrementVotes(rid, 1)
            .then(() => responseModel.addVote(rid, newVote))
            .then(resp => res.json(resp));
        } else {
          res.sendStatus(404);
